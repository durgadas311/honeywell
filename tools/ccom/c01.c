...
int op;
{
	...
	if (dope&ASSGOP || op==CAST) {
		...
		if (op>=LESSEQ && (t1>=PTR||t2>=PTR||t1==UNSIGN||t1==UNLONG||t2==UNSIGN||t2==UNLONG)
		 && (t>=PTR||t==INT||t==CHAR||t==UNSIGN||t==UNLONG))
			op += LESSEQP-LESSEQ;
		if (cvn==ITP || cvn==PTI)
			cvn = 0;
	}
...
int n;
{
	...
	if (Tblock_use + n > Tblock_len) {
		error("Out of space");
		exit(1);
	}
	p = Tblock_base + Tblock_use;
	Tblock_use += n;
	return(p);
}

/* caller MUST treat this as opaque! */
char *
starttree()
{
	return((char *)(long)Tblock_use);
}
...
char *tp;
{
	Tblock_use = (int)(long)tp;
}

static char *Dblock_base = NULL;
static int Dblock_len = 65536;
static int Dblock_use = 0;
/*
...
int n;
{
	...
	if (Dblock_base == NULL) {
		Dblock_base = (char *)malloc(Dblock_len);
		if (Dblock_base == NULL) {
			error("Out of memory");
			exit(1);
		}
	}
	if (Dblock_use + n > Dblock_len) {
		error("Out of space");
		exit(1);
	}
	p = Dblock_base + Dblock_use;
	Dblock_use += n;
	return(p);
}
